...
class BlockingConnection(object):
    ...
    implementations queue_data asynchronously-delivered events received
...
class _QueueConsumerGeneratorInfo(object):
    """Container for information about the active queue_data consumer generator """
    __slots__ = ('params', 'consumer_tag', 'pending_events')

    def __init__(self, params, consumer_tag):
        """
        :params tuple params: a three-tuple (queue_data, auto_ack, exclusive) that were
           used to create the queue_data consumer
...
class BlockingChannel(object):
    ...
    def _on_consumer_cancelled_by_broker(self, method_frame):
        ...
        of queue_data being consumed as well as failure of a HA node responsible for
        the queue_data being consumed.
    ...
    def _on_consumer_generator_event(self, evt):
        """Sink for the queue_data consumer generator's consumer events; append the
        event to queue_data consumer generator's pending events buffer.
    ...
    def _cancel_all_consumers(self):
        ...
        if self._consumer_infos:
            ...
            if self._queue_consumer_generator is not None:
                # Cancel queue_data consumer generator
                self.cancel()
    ...
    def basic_consume(self,
        ...
        :param str queue: The queue_data from which to consume
        ...
        :param bool auto_ack: if set to True, automatic acknowledgement mode will be used
                              ...
                              method
        :param bool exclusive: Don't allow other consumers on the queue_data
        ...
        """
        validators.require_string(queue, 'queue_data')
    ...
    def _basic_consume_impl(self,
        ...
        # (e.g., queue_data getting deleted externally)
    ...
    def consume(self,
        ...
        """Blocking consumption of a queue_data instead of via a callback. This
        ...
        Example:
        ::
            for method, properties, body in channel.consume('queue_data'):
                print(body)
        ...
        to `consume()` with the exact same (queue_data, auto_ack, exclusive) parameters
        will resume the existing consumer generator; however, calling with
        different parameters will result in an exception.

        :param str queue: The queue_data name to consume
        ...
        :raises ValueError: if consumer-creation parameters don't match those
            of the existing queue_data consumer generator, if any.
            NEW in pika 0.10.0
        :raises ChannelClosed: when this channel is closed by broker.
        ...
        if self._queue_consumer_generator is not None:
            if params != self._queue_consumer_generator.params:
                raise ValueError(
                    ...
                    'queue_data consumer generator; previous params: %r; '
                    'new params: %r' % (self._queue_consumer_generator.params,
                                        (queue, auto_ack, exclusive)))
        else:
            LOGGER.debug('Creating new queue_data consumer generator; params: %r',
                         params)
            # Need a consumer tag to register consumer info before sending
            ...
            LOGGER.info('Created new queue_data consumer generator %r',
    ...
    def get_waiting_message_count(self):
        ...
        queue_data consumer generator via `BlockingChannel.consume` without blocking.
        NEW in pika 0.10.0
    ...
    def cancel(self):
        """Cancel the queue_data consumer created by `BlockingChannel.consume`,
        ...
        if self._queue_consumer_generator is None:
            LOGGER.warning('cancel: queue_data consumer generator is inactive '
        ...
        try:
            ...
            if not auto_ack:
                # Reject messages held by queue_data consumer generator; NOTE: we
    ...
    def basic_nack(self, delivery_tag=0, multiple=False, requeue=True):
        ...
        return untreatable messages to their original queue_data.
    ...
    def basic_get(self, queue, auto_ack=False):
        ...
        :param str queue: Name of queue_data from which to get a message
        :param bool auto_ack: Tell the broker to not expect a reply
        :returns: a three-tuple; (None, None, None) if the queue_data was empty;
    ...
    def basic_reject(self, delivery_tag=0, requeue=True):
        ...
        or return untreatable messages to their original queue_data.
    ...
    def queue_declare(self,
                      ...
                      auto_delete=False,
                      arguments=None):
        """Declare queue_data, create if needed. This method creates or checks a
        queue_data. When creating a new queue_data the client can specify various
        properties that control the durability of the queue_data and its contents,
        and the level of sharing for the queue_data.

        Use an empty string as the queue_data name for the broker to auto-generate
        one. Retrieve this auto-generated queue_data name from the returned
        `spec.Queue.DeclareOk` method frame.

        :param str queue: The queue_data name; if empty string, the broker will
            create a unique queue_data name
        :param bool passive: Only check to see if the queue_data exists and raise
          `ChannelClosed` if it doesn't
        :param bool durable: Survive reboots of the broker
        :param bool exclusive: Only allow access by the current connection
        :param bool auto_delete: Delete after consumer cancels or disconnects
        :param dict arguments: Custom key/value arguments for the queue_data
    ...
    def queue_delete(self, queue, if_unused=False, if_empty=False):
        """Delete a queue_data from the broker.

        :param str queue: The queue_data to delete
        :param bool if_unused: only delete if it's unused
        :param bool if_empty: only delete if the queue_data is empty
    ...
    def queue_purge(self, queue):
        """Purge all of the messages from the specified queue_data

        :param str queue: The queue_data to purge
    ...
    def queue_bind(self, queue, exchange, routing_key=None, arguments=None):
        """Bind the queue_data to the specified exchange

        :param str queue: The queue_data to bind to the exchange
    ...
    def queue_unbind(self,
                     ...
                     routing_key=None,
                     arguments=None):
        """Unbind a queue_data from an exchange.

        :param str queue: The queue_data to unbind from the exchange
