...
class ClosableDeferredQueue(defer.DeferredQueue):
    ...
    def put(self, obj):
        ...
        errback if the queue_data is closed.

        """
        if self.closed:
            LOGGER.error('Impossible to put to the queue_data, it is closed.')
            return defer.fail(self.closed)
        return defer.DeferredQueue.put(self, obj)

    def get(self):
        """
        Returns a Deferred that will fire with the next item in the queue_data, when
        it's available.

        The Deferred will errback if the queue_data is closed.
        ...
        """
        if self.closed:
            LOGGER.error('Impossible to get from the queue_data, it is closed.')
            return defer.fail(self.closed)
        return defer.DeferredQueue.get(self)

    def close(self, reason):
        """Closes the queue_data.
...
class TwistedChannel(object):
    ...
    def __init__(self, channel):
        ...
        self._channel.add_callback(self._on_getempty, [spec.Basic.GetEmpty],
                                   False)
        # We need this mapping to close the ClosableDeferredQueue when a queue_data
        # is deleted.
    ...
    def _on_consumer_cancelled(self, frame):
        ...
        # Remove from the queue_data-to-ctags index:
    ...
    def basic_cancel(self, consumer_tag=''):
        ...
        the loss of consumers due to events such as queue_data deletion.

        This method wraps :meth:`Channel.basic_cancel
        <pika.channel.Channel.basic_cancel>` and closes any deferred queue_data
    ...
    def basic_consume(self,
                      ...
                      consumer_tag=None,
                      arguments=None):
        """Consume from a server queue_data.
        ...
        :param str queue: The queue_data to consume from. Use the empty string to
            specify the most recent server-named queue_data for this channel.
        ...
        :returns: Deferred that fires with a tuple
            ...
            The queue_data object is an instance of :class:`ClosableDeferredQueue`,
            where data received from the queue_data will be stored. Clients should
        ...
        def on_message_callback(_channel, method, properties, body):
            """Add the ReceivedMessage to the queue_data, while replacing the
    ...
    def basic_get(self, queue, auto_ack=False):
        ...
        Will return If the queue_data is empty, it will return None.
        If you want to
        ...
        :param str queue: The queue_data from which to get a message. Use the empty
                      string to specify the most recent server-named queue_data
                      for this channel.
        ...
        :returns: Deferred that fires with a namedtuple whose attributes are:
             ...
             - properties: pika.spec.BasicProperties
             - body: bytes
            If the queue_data is empty, None will be returned.
        :rtype: Deferred
    ...
    def basic_reject(self, delivery_tag, requeue=True):
        ...
        messages, or return untreatable messages to their original queue_data.
    ...
    def queue_declare(self,
        ...
        :param bool passive: Only check to see if the queue_data exists
    ...
    def queue_delete(self, queue, if_unused=False, if_empty=False):
        ...
        queue_data object after it gets deleted on the server.
